...
const Vendors = () => {
  ...
  /* ===============================
     ✅ Redux Vendors State
  =============================== */
  const { vendorList = [], isLoading: vendorsLoading } = useSelector((state) => state.vendors);
  const { projects = [], isLoading: projectsLoading } = useSelector((state) => state.projects);

  const isLoading = vendorsLoading || projectsLoading;
  ...
  const filteredVendors = React.useMemo(() => {
    ...
    if (selectedCompany) {
      data = data.filter((v) => v.company_name === selectedCompany);
    }

    if (selectedCategory) {
      data = data.filter(
        (v) => Array.isArray(v.categories) && v.categories.includes(selectedCategory)
      );
    }

    if (selectedProject) {
      data = data.filter(
        (v) => Array.isArray(v.projects) && v.projects.includes(selectedProject)
      );
    }

    return data;
  }, [vendorList, selectedCompany, selectedCategory, selectedProject]);

  const uniqueCompanies = React.useMemo(() => {
    const companies = vendorList
      .filter(Boolean)
      ...
      <div className="flex flex-col gap-4 p-3">
        ...
        <div className="flex flex-wrap justify-between gap-3 bg-bg-card rounded-xl p-3">
          ...
          <div className="flex gap-3 flex-wrap items-end">
            {/* Filter by Company */}
            <div>
              <label className="text-[10px] font-bold uppercase block text-text-muted">
                Company
              </label>
              <select
                ...
                className="mt-1 border border-border-main rounded-xl px-3 py-2 text-xs w-[150px] bg-bg-main text-text-main cursor-pointer outline-none focus:border-primary"
              >
                <option value="" className="bg-bg-card">All Companies</option>
                {uniqueCompanies.map((company) => (
                  <option key={company} value={company} className="bg-bg-card">
                    {company}
                  </option>
                ))}
              </select>
            ...
            <div>
              ...
              <select
                ...
                className="mt-1 border border-border-main rounded-xl px-3 py-2 text-xs w-[150px] bg-bg-main text-text-main cursor-pointer outline-none focus:border-primary"
              >
                <option value="" className="bg-bg-card">All Projects</option>
                {uniqueProjects.map((proj) => (
                  <option key={proj} value={proj} className="bg-bg-card">
                    {proj}
                  </option>
                ))}
              </select>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-bg-main/40 text-[10px] uppercase">
                      ...
                      <th className="px-5 py-4 text-left">Categories</th>
                      <th className="px-5 py-4 text-left">Projects</th>
                  ...
                  <tbody>
                    {paginatedVendors.map((vendor) => (
                      ...
                      >
                        ...
                        {/* Projects List */}
                        <td className="px-5 py-4 text-xs">
                          {Array.isArray(vendor?.projects) &&
                          vendor.projects.length > 0 ? (
                            <ul className="list-disc ml-4 space-y-1">
                              {vendor.projects.slice(0, 3).map((proj, i) => (
                                <li key={i}>{proj}</li>
                              ))}
                            </ul>
                          ) : (
