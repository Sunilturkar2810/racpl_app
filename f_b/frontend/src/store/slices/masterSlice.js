...
export const fetchVendorsCategories = createAsyncThunk(
  "master/fetchVendorsCategories",
  async (_, { getState, rejectWithValue }) => {
    const { token } = getState().auth;
    try {
      const response = await axios.get(`${API_URL}/vendor-categories`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      return response.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Failed to fetch vendors categories",
      );
    }
  },
);
...
const masterSlice = createSlice({
  ...
  builder
    ...
    // vendorsCategories
    .addCase(fetchVendorsCategories.pending, (state) => {
      if (state.vendorsCategories.length === 0) state.isLoading = true;
      state.error = null;
    })
    .addCase(fetchVendorsCategories.fulfilled, (state, action) => {
      state.isLoading = false;
      state.vendorsCategories = action.payload;
      state.lastFetchedDetails.vendorCategories = Date.now();
    })
    .addCase(fetchVendorsCategories.rejected, (state, action) => {
      state.isLoading = false;
      state.error = action.payload;
    })
